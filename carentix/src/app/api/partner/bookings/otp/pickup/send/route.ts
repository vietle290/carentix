...
export async function POST(req: NextRequest) {
    try {
        ...
        if (booking.user.email) {
            await sendMail(booking.user.email, "Your Pickup OTP - CARENTIX", `
                <div style="font-family:sans-serif;padding:20px">
                    <h2>Ride OTP</h2>
                    <p>Your Pickup OTP is:</p>
                    <h1 style="letter-spacing:6px">${otp}</h1>
                    <p>This OTP is valid for 5 minutes.</p>
                    <p>Share this OTP with the driver to start the ride</p>
                    <br/>
                    <b>CARENTIX</b>
                </div>
                `)
        ...
        return NextResponse.json({ message: `Error sending pickup OTP ${error}` }, { status: 500 });
    }
}